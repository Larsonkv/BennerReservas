(function ($) {
    ...
    $.fn.focusOnFirstField = function () {
        return this.each(function () {
            ...
            let allowFocus = null;
            if ($(this).attr("widget-type") === "AjaxForm")
                allowFocus = $(this).find('.allow-focus .active');
            else
                allowFocus = $(this).find('.allow-focus');
            ...
            if (allowFocus) {
                ...
                if (elementFocus.length > 0 && elementFocus.attr("name").endsWith("_select") && elementFocus.is('[multiple]')) {
                    elementFocus = elementFocus.parent().find('input:first');
                }
...
Object.extend = function extend(obj, src) {
    for (var key in src) {
