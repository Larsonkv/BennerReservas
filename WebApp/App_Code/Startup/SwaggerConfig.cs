    ...
    public static class SwaggerConfig
    {
            ...
            GlobalConfiguration.Configuration
                ...
                .EnableSwaggerUi(
                    c =>
                    {
                        ...
                        c.UImaxDisplayedTags(10000);

                        if(DisableTryItOutSwagger())
                            c.SupportedSubmitMethods(new string[] { });
                    });
        }

        /// <summary>
        /// Quando é ambiente de produção, é desabilitado o Try It Out do Swagger, impedindo que ele manipule as APIS e coloque informações indevidas via Swagger na base de produção.
        /// </summary>
        /// <returns></returns>
        private static bool DisableTryItOutSwagger()
        {
            try
            {
                return !(BennerEnvironment.IsProduction && !BennerEnvironment.IsBenner);
            }
            catch
            {
                return true;
        ...
        private class ApplyDocumentVendorExtensions : IDocumentFilter
        {
            ...
            private void GenerateOAuth2Operation(SwaggerDocument swaggerDoc, SchemaRegistry schemaRegistry, string applicationBase)
            {
                ...
                swaggerDoc.paths.Add("/app_services/auth.oauth2.svc/revoke_token", new PathItem
                {
                    post = new Operation
                    {
                        tags = new string[] { "Authentication" },
                        summary = "API para revogar um token de autenticação",
                        description = "O token uma vez revogado não poderá ser reativado, devendo ser descartado",
                        operationId = "OAuth2RevokeTokenPost",
                        ...
                        parameters = new List<Swagger.Net.Parameter>
                        {
                            new Parameter
                            {
                                ...
                                type = "string",
                                required = true,
                                description = "o token que a ser revogado"
                            },
                        },
                        responses = new Dictionary<string, Response>
                        {
